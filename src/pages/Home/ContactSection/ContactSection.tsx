...
const ContactSection: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://bikkhatobazar.magneticcodes.com/api/contact_us", {
        method: "POST",
        headers: {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-32 px-6 py-3 bg-emerald-400 text-white rounded-lg hover:bg-emerald-500 transition-colors"
            >
              {loading ? "Submitting..." : "SUBMIT"}
            </button>
            {responseMessage && (
              <p className="text-sm text-emerald-600 font-medium">{responseMessage}</p>
            )}
          </form>
        </div>
      </div>
